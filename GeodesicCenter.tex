...
Therefore, to find its minimum we can use standard optimization techniques based on cuttings in $\mathbb{R}^3$. This procedure is described in Section~\ref{Section:Solving convex optimization poblem}.
...
then the path $\p{u}{v}$ contains either the path $\p{y^*}{x^*}$; see Figure~\ref{fig:Output Sensitive Algorithm} (top). 
...
In this case, because $u'\in \p{u}{x}\cap \p{u}{v}$ by definition and since $y, v$ and $x$ are in clockwise order, the path $\p{u}{v}$ contains $u'$. Analogously, we know also the path $\p{u}{v}$ contains $v'$
...
Recall that each of four paths bounding $P'$ is a shortest geodesic path. 
We claim that $P'$ is a pseudo-quadrilateral with only four convex vertices being $x',u',y'$ and $v'$. 
To show this, recall that each path bounding $P'$ is a shortest path. Thus, if $\p{x'}{u'}$ was not a path containing only reflex vertices of $P'$, then the shortest path from $x'$ to $u'$ restricted to $P'$ would be shorter than $\p{x'}{u'}$ which is  a contradiction since $P'\subseteq P$. Thus, each interior vertex of $\p{x'}{u'}$ is a  reflex vertices of $P'$. The same argument holds for each of the other three paths bounding $P'$. Therefore, $P'$ is a simple pseudo-quadrilateral with $x',u',y'$ and $v'$ as its four convex vertices.
...
We want to find the common tangent from one reflex path to another, namely from either $\p{u'}{x'}$ or $\p{u'}{y'}$ to either $\p{v'}{x'}$ or $\p{v'}{y'}$ as one of them belongs to the shortest path $\p{u'}{v'}$.
Assume that the desired tangent lies between the paths $\p{u'}{x'}$ and $\p{v'}{y'}$, we consider the four possible cases later. 
...
By slightly modifying the linear time algorithm to compute this tangents, we can make it run in $O(\link{u'}{v'})$ time.

Since we do not know if the tangent lies between the paths $\p{u'}{x'}$ and $\p{v'}{y'}$, we process the four pairs of paths in parallel and stop when finding the desired tangent. That is, we can compute $\p{u'}{v'}$ in $O(\link{u'}{v'})$ time and consequently, we can compute $\p{u}{v}$ in $O(\link{u}{v})$ time.
\end{proof}
...
Note that if $v_a \neq a$, then there $v_a$ is inside the triangle $\triangle(a,b,v)$ and the same holds for $v_b$. 
Thus, if a vertex $v$ is visible, then the extension of the  segments $vv_a$ and $vv_b$ must intersect the top segment $ab$ at points $x_a$ and $x_b$, respectively.
...
Because $p\in R(\ff{p})$, Corollary~\ref{corollary:Cell contained in geodesic triangle} implies that $p\in H_{ab}$. 
Consider the path $\p{p}{\ff{p}}$ and let $v$ be the successor of $p$ along this path. Since $p\in R(\ff{p})$, the ray shooting from $v$ towards $p$ stays within $R(\ff{p})$ which means that it must intersect the segment $ab$. That is, $v$ is visible from $ab$ and $p$ lies inside one of the apex triangles defined by $v$, i.e., there is a triangle $\triangle\in \mathcal A_{ab}$ apexed at $v$ with some definer $w$ that contains $p$.
Since the apex function of $\triangle$ encodes the geodesic distance to a vertex $w$ and
because $\F{P}{x}$ is the upper envelope of all the geodesic functions, we know that $g(p) \leq \F{P}{p}$.

%By construction of $\mathcal A_{ab}$, there is a triangle $\triangle\in \mathcal A_{ab}$ apexed at $v$ with definer $w$ that contains $p$. The apex function $g(x)$ of $\triangle$ encodes the geodesic distance from $x$ to $w$. Because $\F{P}{x}$ is the upper envelope of all the geodesic functions, we know that $g(p) \leq \F{P}{p}$.
...
We spend the remainder of this section proving that $A$ is not shattered by $\mathcal H$.
 ...
 A $6$-cell is \emph{strict} if it is defined as the intersection of six half-polygons, none of which are redundant, i.e., the removal of any of them modifies the $6$-cell.
