...
\author[1]{Some Authors \thanks{Grants}}
...
The first algorithm for computing the geodesic center was given by Asano and Toussaint~\cite{at-cgcsp-85}, and runs in $O(n^4\log n)$-time. In 1989, Pollack, Sharir, and Rote~\cite{pollackComputingCenter} improved it to $O(n\log n)$ time. Since then, it has been a open problem whether the geodesic center can be
...
We modify the tree $T_v$ by removing the edge $w_1w_2$ and adding the edge $w_3s$ (no edge is added if $w_3 = s$); see Figure~\ref{fig:Funnel Cover} for an illustration. \mati{For reasoning purposes, isn't it simpler to remove $w_1$ AND $w_2$ and add $s$? This reduces the size of $T$} \luis{note that $w_2$ can have more children,  so we cannot remove it. Consider the next step of the algorithm after the sixth figure}

\begin{figure}[tb]
\centering
\includegraphics{img/FunnelCover.pdf}

\caption{\small The funnel $\fn{v}{C_{v}}$ $(a)$ and the shortest-path tree from $v$ $(b)$ are depicted.
Figures $c, d, e$ and $f$ depict two cases of the algorithm described in Lemma~\ref{lemma:Triangles inside funnels}. 
In the first step, $w_1,w_2,w_3$ makes a right turn $(c)$ while in the second $(e)$ they make a left turn.}
\label{fig:Funnel Cover}
\end{figure}

\textbf{Case 2.} If $w_1, w_2, w_3$ makes a right turn, then let $s$ be the first point hit by the ray apexed at $w_2$ that shoots in the direction opposite to $w_3$\mati{again, I think htat w2 and w3 are reversed} \luis{I don't see this. In the figures we have first Case 2 where $w_1,w_2,w_3$ make a right turn and then Case 1 where they make a left turn in the 5th drawing}. By the same argument as above, we can show that $w_1$ and $s$ lie on the same edge of the boundary of $\fn{v}{C_v}$. Therefore, we can compute $s$ in $O(1)$ time. At this point, we construct the apexed triangle $\triangle(w_1, w_2, s)$ apexed at $w_2$ with apex function $g(x) = |x w_2| + \g{w_2}{v}$.
...
Let $m = \max\{|R|, |\tau_R|\}$.\mati{$m$ should depend on $R$?}\luis{Yes, $m$ is redefined in each iteration, it is independent of $n$}
...
\mati{This whole paragraph is very confusing....and not really needed. Again, I do not like $R$-trapezoid. How about geodesic trapezoid?}
\luis{It depends on $R$ so it should be in the name. This paragraph is needed to define the range space from which we take the epsilon net, I don't see how to avoid it}
...
Because our decomposition into cells has constant complexity, 
...
In order to proceed with the algorithm on  $\cell$ recursively, we need to compute the set $\tau_{\cell}$ of at most $\varepsilon |\mathcal C|$ apexed triangles of $\tau_R$ that intersect $\cell$. We proceed as follows.
...
Since $|\mathcal C| \leq 2m$, we guarantee that at most $2\varepsilon m$ apexed triangles intersect~$\cell$. 
Moreover, because each vertex of $\cell$ is in at least one apexed triangle of $\tau_R$ and from the fact that each apexed triangle covers at most three vertices, we conclude that $\cell$ consists of at most $6 \varepsilon m$ vertices. \mati{you mean at most $6 \varepsilon m$ apexed triangles intersect with $\cell$?}\luis{No, $6\varepsilon m$ vertices}
...
The important thing to notice is that, as in the case of chords, the function $\phi(x)$ restricted to $\cell$ is a convex function, which allows us to do prune and search using cuttings.
...
$$g_i^2(x) = \|x\|^2 + 2x\cdot a_i + \|a_i\|^2  = |x a_i|^2 \leq (r - \kappa_i)^2 = r^2 - 2r\kappa_i + \kappa_i^2.$$ 
...
Using this side-decision algorithm, he shows how to solve the optimization problem. A reinterpretation of his technique could be described as follows: Start by pairing the functions arbitrarily, and then consider the set of separating planes defined by these pairs.
For some constant $r$, compute a $1/r$-cutting in $\mathbb{R}^3$ of the separating planes.
An $1/r$-cutting is a partition of the plane into $O(r^2)$ convex regions each of which is of constant size and intersects at most $n/r$ separating planes.
...
Because at least $(r-1)n/r$ separating planes do not intersect this constant size region, for each of them we can discard one of the constraints as it becomes redundant. Repeating this algorithm recursively we obtain a linear running time.
...
Once the minimum of (P2) restricted to $\gamma$ is known, we can follow the same approach used by Megiddo~\cite{megiddo1989ball} to find the side of $\gamma$ containing the global minimum. 
